...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      setAuthError(err.response?.data?.detail || 'Registration failed. Try a different username.');
    }
  };
  ...
  return (
    <Box sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'background.default', p: 2 }}>
      <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, overflow: 'hidden', zIndex: 0, opacity: 0.4 }}>
        <Box sx={{ position: 'absolute', top: '-10%', right: '-10%', width: '40%', height: '40%', borderRadius: '50%', background: 'radial-gradient(circle, rgba(236,72,153,0.3) 0%, rgba(15,23,42,0) 70%)', filter: 'blur(40px)' }} />
        <Box sx={{ position: 'absolute', bottom: '-10%', left: '-10%', width: '40%', height: '40%', borderRadius: '50%', background: 'radial-gradient(circle, rgba(99,102,241,0.4) 0%, rgba(15,23,42,0) 70%)', filter: 'blur(40px)' }} />
      </Box>
      ...
      <Paper elevation={0} sx={{ p: { xs: 4, md: 6 }, width: '100%', maxWidth: 480, zIndex: 1, borderRadius: 4, border: '1px solid #334155', background: 'rgba(30, 41, 59, 0.7)', backdropFilter: 'blur(20px)' }}>
        ...
        <Typography variant="body2" align="center" color="text.secondary">
          ...
          <MuiLink component={Link} to="/login" color="primary.main" underline="hover" fontWeight="bold">
            Sign In
          </MuiLink>
        </Typography>
      </Paper>
